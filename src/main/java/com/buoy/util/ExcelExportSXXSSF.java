...
public class ExcelExportSXXSSF {
	...
	public static ExcelExportSXXSSF start(String filePrefix,
			List<String> fieldNames,List<String> fieldCodes, int flushRows) throws Exception {
		ExcelExportSXXSSF excelExportSXXSSF = new ExcelExportSXXSSF();
		//excelExportSXXSSF.setFilePath(filePath);
		//excelExportSXXSSF.setFileWebPath(fileWebPath);
	...
	private void writeTitles(String sheetName) throws Exception {
		rownum = 0;//第0行
		colnum = fieldNames.size();//根据列标题得出列数
		Row row = getWb().getSheet(sheetName).createRow(rownum);
		for (int cellnum = 0; cellnum < colnum; cellnum++) {
			Cell cell = row.createCell(cellnum);
			cell.setCellValue(fieldNames.get(cellnum));
		}
	...
	public void writeDatasByObject(List datalist,String sheetName) throws Exception {
		Sheet sheet = getWb().createSheet(sheetName);
		//设置列宽
		sheet.setColumnWidth(0, 20 * 256); //50个字符大小
		sheet.setColumnWidth(1, 20 * 256);
		sheet.setColumnWidth(2, 150 * 256);
		writeTitles(sheetName);
		for (int j = 0; j < datalist.size(); j++) {
...
//			Row row = sh.createRow(rownum);
	...
	private Object invokeMethod(Object owner, String fieldname, Object[] args)
		...
		String methodName = StringUtil.changeToGetXXX(fieldname);
