 ...
 */
  ...
  (function() {
      ...
      AreaRestriction.setBounds = function(map, bounds){
          ...
          _map.addEventListener("dragend", this._mapMoveendEvent);
          _isRestricted = true;
          return true;
      };
      ...
      AreaRestriction._mapMoveendEvent = function(e) {
        ...
        // 重新设置回初始的点（折中方案，不然利用drag事件+重写center这个变量，要花时间慢慢调。。。。）
        // move事件有个弊端就是和很容易与infoWindow的enableAutoPan事件冲突
		center = new BMap.Point(111.735690, 15.229647);
     ...
     AreaRestriction.clearBounds = function(){
         ...
         _map.removeEventListener("dragend", this._mapMoveendEvent);
         _isRestricted = false;
     };
